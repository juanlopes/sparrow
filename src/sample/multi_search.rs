...
pub fn multi_search_placement(l: &Layout, item: &Item, ref_pk: Option<PItemKey>, mut evaluator: impl SampleEvaluator, search_config: SearchConfig, rng: &mut impl Rng) -> (DTransformation, SampleEval) {
    /*
          Using the threadpool, search for placements of multiple items at once.
          Each time execute the cheapest modification.
          Each worker has its own evaluator and best_samples instance.

          From one iteration to another, the best samples are kept (but re-evaluated).
     */
