...
pub fn main() {
        ...
        for sy in 1..RESOLUTION-1 {
            if let Overlap::None = overlaps[sx * RESOLUTION + sy] {
                ...
                if neighbors.iter().any(|(nx, ny)| {
                    matches!(overlaps[*nx * RESOLUTION + *ny], Overlap::Items(_))
                }) {
...

    {
        ...
        let z: Vec<Vec<f64>> = overlaps.iter()
            .map(|o| match o {
        ...
        let x = (0..RESOLUTION).map(|i| bbox.x_min + bbox.width() / RESOLUTION as fsize * i as fsize).collect::<Vec<fsize>>();
        let y = (0..RESOLUTION).map(|i| bbox.y_min + bbox.height() / RESOLUTION as fsize * i as fsize).collect::<Vec<fsize>>();
...
pub fn poly_overlap_proxy(s1: &SimplePolygon, s2: &SimplePolygon) -> fsize {
    ...
    let penalty = 1.00 * fsize::min(s1_penalty, s2_penalty) + 0.00 * fsize::max(s1_penalty, s2_penalty);
