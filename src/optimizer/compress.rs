...
use log::info;
use rand::Rng;
use crate::config::COMPRESS_SHRINK_RANGE;
    ...
    let shrink_step_size = |n_failed_attempts: i32| -> f32 {
        match decay {
            ShrinkDecayStrategy::TimeBased(end) => {
                ...
                let ratio = elapsed.as_secs_f32() / (end.duration_since(start)).as_secs_f32();
                COMPRESS_SHRINK_RANGE.0 + ratio * range
            }
            ShrinkDecayStrategy::FailureBased(r) => {
                COMPRESS_SHRINK_RANGE.0 * r.powi(n_failed_attempts)
            }
        }
    };
    while !term.kill() && let step = shrink_step_size(n_failed_attempts) && step >= COMPRESS_SHRINK_RANGE.1 {
        match attempt_to_compress(sep, &best, step, term, sol_listener) {
            Some(compacted_sol) => {
                info!("[CMPR] successfully compressed by {:.3}% to {:.3} ({:.3}%)", step * 100.0, compacted_sol.strip_width(), compacted_sol.density(instance) * 100.0);
                sol_listener.report(ReportType::CmprFeas, &compacted_sol, instance);
                best = compacted_sol;
            }
            None => {
                info!("[CMPR] failed to compress by {:.3}%", step * 100.0);
                n_failed_attempts += 1;
            }
        }
    }
